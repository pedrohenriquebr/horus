...
public class Filter<T>
{
    ...
    /// <summary>
    ///     Creates a new Bloom filter, specifying an error rate of 1/capacity, using the optimal size for the underlying data
    ///     structure based on the desired capacity and error rate, as well as the optimal number of hash functions.
    ///     A secondary hash function will be provided for you if your type T is either string or int. Otherwise an exception
    ///     will be thrown. If you are not using these types please use the overload that supports custom hash functions.
    /// </summary>
    /// <param name="capacity">
    ///     The anticipated number of items to be added to the filter. More than this number of items can be
    ///     added, but the error rate will exceed what is expected.
    /// </param>
    ...
    /// <summary>
    ///     Creates a new Bloom filter, using the optimal size for the underlying data structure based on the desired capacity
    ///     and error rate, as well as the optimal number of hash functions.
    ///     A secondary hash function will be provided for you if your type T is either string or int. Otherwise an exception
    ///     will be thrown. If you are not using these types please use the overload that supports custom hash functions.
    /// </summary>
    /// <param name="capacity">
    ...
    ///     The function to hash the input values. Do not use GetHashCode(). If it is null, and T is
    ///     string or int a hash function will be provided for you.
    /// </param>
    ...
    public Filter(int capacity, float errorRate, HashFunction hashFunction, int m, int k)
    {
        ...
        if (errorRate >= 1 || errorRate <= 0)
            throw new ArgumentOutOfRangeException(
                ...
                errorRate,
                string.Format(
                    "errorRate must be between 0 and 1, exclusive. Was {0}",
                    errorRate
                )
            );
    ...
    public bool Contains(T item)
    {
        ...
        {
            var hash = ComputeHash(primaryHash, secondaryHash, i);
            if (_hashBits[hash] == false) return false;
        }
    ...
    {
        return (int)
            Math.Ceiling(capacity * Math.Log(errorRate, 1.0 / Math.Pow(2, Math.Log(2.0))));
    }
    ...
    private static int HashInt32(T input)
    {
        ...
        {
            x = ~x + (x <<
                      15); // x = (x << 15) - x- 1, as (~x) + y is equivalent to y - x - 1 in two's complement representation
            x = x ^ (x >> 12);
            x = x + (x << 2);
            x = x ^ (x >> 4);
